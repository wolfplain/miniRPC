...
static std::unordered_map<EventType, int> g_eventMap = {
    {EventType::READ, EPOLLIN}, {EventType::WRITE, EPOLLOUT}, {EventType::RW, EPOLLRDHUP},
    {EventType::ET, EPOLLET},   {EventType::HUP, EPOLLRDHUP},
};
...
bool Event::IsReadEvent(int eventId) {
    if (eventId >= MAX_EVENTS) { return false; }
...
int Event::GetConnectFd(int eventId) {
    if (eventId >= MAX_EVENTS) { return -1; }
